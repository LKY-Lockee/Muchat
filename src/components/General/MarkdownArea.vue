...
interface Props {
  ...
  onThinkCollapse?: EventListenerOrEventListenerObject
  thinkCollapsed?: boolean
}

const props = withDefaults(defineProps<Props>(), {
  onThinkCollapse: undefined,
})
...
function attachListeners() {
  ...
  thinkExpandBtns?.forEach((think) => {
    if (!props.onThinkCollapse) return
    think.removeEventListener('click', props.onThinkCollapse)
    think.addEventListener('click', props.onThinkCollapse)
  })
}
...
onMounted(async () => {
  ...
  if (props.thinkCollapsed) {
    ...
    thinkElements?.forEach(async (thinkElement) => {
      ...
      thinkElement.classList.add('collapsed')
    })
  }
})
